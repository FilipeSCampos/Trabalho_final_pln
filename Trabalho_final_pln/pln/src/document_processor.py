...
# Compat shim: algumas versões de langchain_core esperam que o pacote
# `langchain` tenha o atributo `verbose`. Em instalações mistas isso pode
# não existir e causar AttributeError ao instanciar modelos. Definimos um
# fallback seguro antes de importar componentes que dependem de langchain.
try:
    langchain_mod = importlib.import_module("langchain")
    if not hasattr(langchain_mod, "verbose"):
        setattr(langchain_mod, "verbose", False)
except Exception:
    # Se o import falhar, ignoramos — as importações seguintes irão
    # levantar o erro original e devem ser tratadas normalmente.
    pass
