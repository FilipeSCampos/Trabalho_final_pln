    ...
    def get_knowledge_sources_info(self, source_names: List[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for source in selected_sources:
                ...
                # Tentar obter provider do model_config
                provider = model_config.get("provider", "unknown")
                
                # Se provider não está definido, tentar inferir pelo embedding_model
                if provider == "unknown":
                    if embedding_model == "openai" or "openai" in str(embedding_model).lower():
                        provider = "openai"
                    elif embedding_model == "gemini" or "gemini" in str(embedding_model).lower():
                        provider = "gemini"
                    # Fallback: verificar pelo nome da collection
                    elif "gemini" in source["name"].lower():
                        provider = "gemini"
                    elif "openai" in source["name"].lower() or "curriculos" in source["name"].lower():
                        # Collections de currículos geralmente são OpenAI
                        provider = "openai"
                    else:
                        # Default: assumir OpenAI se não conseguir determinar
                        provider = "openai"
                ...
                source_info = {
                    ...
                    "vector_dimension": model_config.get("dimension", 0),
